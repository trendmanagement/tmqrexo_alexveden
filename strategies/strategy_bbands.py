...
class StrategyBollingerBands(StrategyBase):
    ...
    def calc_entry_rules(self, bb_period, bb_k, rules_index):
        ...
        if rules_index >= 14:
            ...
            bb_multiperiod_width_pctrank = df['bb_multiperiod_width'].expanding().apply(lambda x: pctrank(x))
            bb_multiperiod_pctb_pctrank = df['bb_multiperiod_%b'].expanding().apply(lambda x: pctrank(x))
        ...
        if rules_index <= 13:
            ...
            bb_width_pctrank = df['bb_width'].expanding().apply(lambda x: pctrank(x))
            bb_pctb_pctrank = df['bb_%b'].expanding().apply(lambda x: pctrank(x))
        ...
        if rules_index == 2:
            bb_width_pctrank_less_20 = bb_width_pctrank <= 0.2

            return bb_width_pctrank_less_20

        if rules_index == 3:
            bb_width_pctrank_less_10 = bb_width_pctrank <= 0.1

            return bb_width_pctrank_less_10

        if rules_index == 4:
            bb_width_pctrank_more_80 = bb_width_pctrank >= 0.8

            return bb_width_pctrank_more_80

        if rules_index == 5:
            bb_width_pctrank_more_90 = bb_width_pctrank >= 0.9

            return bb_width_pctrank_more_90

        if rules_index == 6:
            bb_width_pctrank_more_50 = bb_width_pctrank >= 0.5

            return bb_width_pctrank_more_50

        if rules_index == 7:
            bb_width_pctrank_less_50 = bb_width_pctrank <= 0.5

            return bb_width_pctrank_less_50

        if rules_index == 8:
            bb_pctb_pctrank_less_20 = bb_pctb_pctrank <= 0.2

            return bb_pctb_pctrank_less_20

        if rules_index == 9:
            bb_pctb_pctrank_less_10 = bb_pctb_pctrank <= 0.1

            return bb_pctb_pctrank_less_10

        if rules_index == 10:
            bb_pctb_pctrank_more_80 = bb_pctb_pctrank >= 0.8

            return bb_pctb_pctrank_more_80

        if rules_index == 11:
            bb_pctb_pctrank_more_90 = bb_pctb_pctrank <= 0.9

            return bb_pctb_pctrank_more_90

        if rules_index == 12:
            bb_pctb_pctrank_more_50 = bb_pctb_pctrank >= 0.5

            return bb_pctb_pctrank_more_50

        if rules_index == 13:
            bb_pctb_pctrank_less_50 = bb_pctb_pctrank <= 0.5
        ...
        if rules_index == 16:
            bb_multiperiod_width_pctrank_less_20 = bb_multiperiod_width_pctrank <= 0.2

            return bb_multiperiod_width_pctrank_less_20

        if rules_index == 17:
            bb_multiperiod_width_pctrank_less_10 = bb_multiperiod_width_pctrank <= 0.1

            return bb_multiperiod_width_pctrank_less_10

        if rules_index == 18:
            bb_multiperiod_width_pctrank_more_80 = bb_multiperiod_width_pctrank >= 0.8

            return bb_multiperiod_width_pctrank_more_80

        if rules_index == 19:
            bb_multiperiod_width_pctrank_more_90 = bb_multiperiod_width_pctrank <= 0.9

            return bb_multiperiod_width_pctrank_more_90

        if rules_index == 20:
            bb_multiperiod_width_pctrank_more_50 = bb_multiperiod_width_pctrank >= 0.5

            return bb_multiperiod_width_pctrank_more_50

        if rules_index == 21:
            bb_multiperiod_width_pctrank_less_50 = bb_multiperiod_width_pctrank <= 0.5

            return bb_multiperiod_width_pctrank_less_50

        if rules_index == 22:
            bb_multiperiod_pctb_pctrank_less_20 = bb_multiperiod_pctb_pctrank <= 0.2

            return bb_multiperiod_pctb_pctrank_less_20

        if rules_index == 23:
            bb_multiperiod_pctb_pctrank_less_10 = bb_multiperiod_pctb_pctrank <= 0.1

            return bb_multiperiod_pctb_pctrank_less_10

        if rules_index == 24:
            bb_multiperiod_pctb_pctrank_more_80 = bb_multiperiod_pctb_pctrank >= 0.8

            return bb_multiperiod_pctb_pctrank_more_80

        if rules_index == 25:
            bb_multiperiod_pctb_pctrank_more_90 = bb_multiperiod_pctb_pctrank <= 0.9

            return bb_multiperiod_pctb_pctrank_more_90

        if rules_index == 26:
            bb_multiperiod_pctb_pctrank_more_50 = bb_multiperiod_pctb_pctrank >= 0.5

            return bb_multiperiod_pctb_pctrank_more_50

        if rules_index == 27:
            bb_multiperiod_pctb_pctrank_less_50 = bb_multiperiod_pctb_pctrank <= 0.5
