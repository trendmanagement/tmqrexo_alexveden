...
class StrategyRenkoPatterns_no_exit_on_patterns_DynamicBoxSize(StrategyBase):
    name = 'RenkoPatternsNoExitsDynBox'
    ...
    def calc_entry_rules(self, atr_period, move_count, rules_index):
        ...
        O = L = H = C = self.data.exo
        period = atr_period

        atr = ATR(H, L, C, period)
        # 
        for i in range(len(self.data.exo)):
            box_size = atr.shift(1)[i]  # OPT PARAM
    ...
    def calculate(self, params=None, save_info=False):
        ...
        if params is None:
            ...
            direction, atr_period, move_count, rules_index, period_median = self.default_opts()
        else:
            # Unpacking optimization params
            #  in order in self.opts definition
            direction, atr_period, move_count, rules_index, period_median = params
        ...
        # Enry/exit rules
        entry_rule = self.calc_entry_rules(atr_period, move_count, rules_index)
