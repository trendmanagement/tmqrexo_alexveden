...
class StrategyIchimokuCloud(StrategyBase):
    ...
    # @numba.jit
    def calc_entry_rules(self, conversion_line_period, base_line_period, leading_spans_lag,
        ...
        '''
        leading_spans_lag = leading_spans_lag  # subject of optimization
        leading_span_a = ((conversion_line + base_line) / 2).shift(leading_spans_lag)

        # straightforward time shifting to 'leading_spans_lag' number of days
        # might be slower than .shift method
        # leading_span_a.index = leading_span_a.index + pd.DateOffset(days=leading_spans_lag)
        ...
        leading_span_b = ((price_df.rolling(window=leading_span_b_period).max() + price_df.rolling(
            window=leading_span_b_period).min()) / 2).shift(leading_spans_lag)

        # straightforward time shifting to 'leading_spans_lag' number of days
        # might be slower than .shift method
        # leading_span_b.index = leading_span_b.index + pd.DateOffset(days=leading_spans_lag)
    ...
    def calculate(self, params=None, save_info=False):
        ...
        if params is None:
            ...
            direction, conversion_line_period, base_line_period, leading_spans_lag, leading_span_b_period, rules_index, period_median = self.default_opts()
        else:
            # Unpacking optimization params
            #  in order in self.opts definition
            direction, conversion_line_period, base_line_period, leading_spans_lag, leading_span_b_period, rules_index, period_median = params
        ...
        entry_rule = self.calc_entry_rules(conversion_line_period, base_line_period,
                                           leading_spans_lag, leading_span_b_period, px, rules_index)
