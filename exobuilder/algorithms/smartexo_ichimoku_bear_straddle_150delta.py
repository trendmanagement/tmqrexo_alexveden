...
def new_position_bullish_zone(date, fut, opt_chain):
    ...
    #
    # Opening long Cond in bullish zone
    #
    ...
    trans_list = [
        ...
        Transaction(opt_chain[6].c, date, -1.0),
    ]
...
def new_position_bearish_zone(date, fut, opt_chain):
    ...
    #
    # Opening ITM Bear staddle in bearish zone
    #

    trans_list = [
                #Transaction(asset, date, qty, price=[MktPrice], leg_name=['' or unique name])
                Transaction(opt_chain[2].c, date, 2.0, leg_name='bearish'),
                Transaction(opt_chain[2].p, date, 4.0),
                ]
...
def new_position_neutral_zone(date, fut, opt_chain):
    ...
    trans_list = [
                #Transaction(asset, date, qty, price=[MktPrice], leg_name=['' or unique name])
                Transaction(opt_chain[-2].c, date, -1.0, leg_name='neutral'),
                Transaction(opt_chain[2].c, date, 2.0),

                Transaction(opt_chain[8].c, date, -2.0),
           ]
